...
def handle_tamara_webhook(request: HttpRequest, config: GatewayConfig, gateway: str):
...
def handle_tamara_authorization(
    request: HttpRequest, config: GatewayConfig, gateway: str
):
    data_from_tamara = json.loads(request.body.decode("utf-8").replace("'", '"'))
